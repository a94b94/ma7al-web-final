// ✅ API: /api/inventory/import-invoice.ts
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    // 1️⃣ إدخال المنتجات إلى المخزن
    const insertedProducts = await InventoryProduct.insertMany(products);

    // 2️⃣ إنشاء سجل الفاتورة وربطه بالمنتجات
    ...
    return res.status(500).json({ message: "⚠️ فشل في الحفظ" });
  }
}
